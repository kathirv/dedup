    ...
    public class SyncRepository : ISyncRepository
    {
        ...
        public static int SyncGoldenTableIsExist(ConnectorConfig connectorConfig)
        {
            ...
            try
            {
                ...
                else if (connectorConfig?.destDBConfig.databaseType == DatabaseType.Heroku_Postgres
                   || connectorConfig?.destDBConfig.databaseType == DatabaseType.Azure_Postgres
                   || connectorConfig?.destDBConfig.databaseType == DatabaseType.AWS_Postgres)
                {
                    if (connectorConfig?.destDBConfig.databaseType != null)
                    {
                        //Remove special chars in table name
                        connectorConfig.destObjectName = Utilities.RemoveSpecialChars(connectorConfig.destObjectName);
                        //db schema
                        ...
                        using (ConnectionFactory connectionFactory = new ConnectionFactory(connectionURL))
                        {
                            using (IDbTransaction transaction = connectionFactory.DbConnection.BeginTransaction())
                            {
                                try
                                {
                                    ...
                                    //sb.Append($"DELETE FROM \"{schemaName}\".\"{tableName}\" c WHERE c.ctid = ANY(ARRAY(SELECT a.ctid FROM \"{schemaName}\".\"{tableName}\" a");
                                    //sb.Append($" LEFT OUTER JOIN(SELECT MAX(ctid) as ctid FROM \"{schemaName}\".\"{tableName}\"");
                                    //sb.Append(" GROUP BY " + string.Join(",", fetchColumns.Select(c => c).ToArray()) + ") b on a.ctid=b.ctid WHERE b.ctid is null))");
                ...
                if (connectorConfig.dbConfig != null)
                {
                    ...
                    if (connectorConfig.dedup_process_count == null || connectorConfig.dedup_process_count <= 0)
                    {
                        using (ConnectionFactory connectionFactory = new ConnectionFactory(connectionURL))
                        {
                            ...
                            // sb.Append($" (scan_time timestamp without time zone,myctid tid,parentctid tid,marked_for_delete boolean,child_count integer);");
                    ...
                    {
                        sb.Clear();
                        sb.Append($" Insert Into \"{schemaName}\".\"{tableName}_ctindex\"(myctid,child_count)");
                        sb.Append($" (SELECT t.ctid,1 FROM(SELECT DISTINCT ON({string.Join(",", fetchColumns.Select(c => c).ToArray()) }) ctid");
                        sb.Append($" FROM \"{schemaName}\".\"{tableName}\") t order by t.ctid);");
                        //Select total count for loop and find duplicates
                        sb.Append($"select count(myctid) AS total_rows from \"{schemaName}\".\"{tableName}_ctindex\" where child_count = 1;");
                        ...
                        //sb.Append($" Update \"{schemaName}\".\"{tableName}_ctindex\" set child_count = 1 where myctid in(");
                    ...
                    if (operators == SimilarityType.Logical_AND || operators == SimilarityType.Logical_OR)
                    {
                        ...
                        if (tableDataSize != null && tableDataSize.total_rows > 0)
                        {
                            ...
                            if (iNoOfPages > 0)
                            {
                                do
                                {
                                    ...
                                    sb.Clear();
                                    sb.Append($"select {string.Join(",", fetchColumns.Select(c => c).ToArray()) },ctid from \"{schemaName}\".\"{tableName}\" where ctid in(select myctid from \"{schemaName}\".\"{tableName}_ctindex\" where child_count is not null order by myctid)");

                                    //sb.Append($"select myctid from \"{schemaName}\".\"{tableName}_ctindex\" where child_count = 1");
                                    ...
                                    if (pgRows != null && pgRows.Count() > 0)
                                    {
                                        using (ConnectionFactory connectionFactory = new ConnectionFactory(connectionURL))
                                        {
                                            ...
                                            for (int i = 0; pgRows.Count() > i; i++)
                                            {
                                                ...
                                                //sb.Append($"INSERT INTO \"{schemaName}\".\"{tableName}_ctindex\"(myctid,parentctid,marked_for_delete) select t1.\"ctid\",'" + ctid + "',true from ");
                                                //sb.Append($"(select ctid,{string.Join(", ", fetchColumns.Select(c => c).ToArray()) } from \"{schemaName}\".\"{tableName}\"");
                                                //sb.Append($" where \"ctid\" not in (select \"myctid\" from \"{schemaName}\".\"{tableName}_ctindex\" where child_count is not null)) t1,");
                                                //sb.Append($"(select ctid,{string.Join(", ", fetchColumns.Select(c => c).ToArray()) } from \"{schemaName}\".\"{tableName}\" where ctid = '" + ctid + "') t2");
                                                //sb.Append(" where ");
                                                ...
                                                //    if (loop_count == 0)
                                                //    {
                                                //        sb.Append($"SIMILARITY(t1.\"{ofr.Similarity_Value}\"::text,t2.\"{ofr.Similarity_Value}\"::text) " + symbool + ofr.Similarity_Percent);
                                                //    }
                                                //    else
                                                //    {
                                                ...
                                                foreach (ObjectFieldsWithRatio ofr in connectorConfig.DedupObjectFields)
                                                {
                                                    if (loop_count == 0)
                                                    {
                                                        sb.Append($" SIMILARITY(\"{ofr.Similarity_Value}\"::text,COALESCE('{pgRows[i][ofr.Similarity_Value]}','')) " + symbool + ofr.Similarity_Percent);
                                                    }
                                                    else
                                                    {
                                                        sb.Append(" " + queryCondition);
                        ...
                        {
                            sb.Clear();
                            sb.Append($"Update \"{schemaName}\".\"{tableName}_ctindex\" c set child_count=null where parentctid is not null;");
                    ...
                    // if (connectorConfig.dedup_process_count == null || connectorConfig.dedup_process_count <= 0)
                    {
                        using (ConnectionFactory connectionFactory = new ConnectionFactory(connectionURL))
                        {
                            sb.Clear();
                            sb.Append($" Insert Into \"{schemaName}\".\"{tableName}_ctindex\"(myctid)");
                            sb.Append($" (SELECT ctid FROM \"{schemaName}\".\"{tableName}\" where ctid not in (SELECT myctid FROM \"{schemaName}\".\"{tableName}_ctindex\"));");
                            //Select total count for loop and find duplicates
                            //sb.Append($"select count(myctid) AS total_rows from \"{schemaName}\".\"{tableName}_ctindex\" where child_count = 1;");
                            WriteProcessLogToConsole(connectorConfig.connectorName, $"Create/Insert query for ctindex table=>{sb.ToString()}");
                            var qryResult = await connectionFactory.DbConnection.QueryAsync<PGTableDataSize>(sb.ToString()).ConfigureAwait(false);
                    ...
                    if (operators == SimilarityType.Logical_AND || operators == SimilarityType.Logical_OR)
                    {
                        ...
                        while (isCtindexNull)
                        {
                            ...
                            sb.Clear();
                            sb.Append($"select {string.Join(",", fetchColumns.Select(c => c).ToArray()) },ctid from \"{schemaName}\".\"{tableName}\" where ctid in(select myctid from \"{schemaName}\".\"{tableName}_ctindex\" where child_count is null and parentctid is null limit 1);");
                            ...
                            if (pgRows != null && pgRows.Count() > 0)
                            {
                                ...
                                using (ConnectionFactory connectionFactory = new ConnectionFactory(connectionURL))
                                {
                                    for (int i = 0; pgRows.Count() > i; i++)
                                    {
                                        string ctid = pgRows[i]["ctid"].ToString();
                                        sb.Append("WITH temp AS(");
                                        sb.Append($"Update \"{schemaName}\".\"{tableName}_ctindex\" a set parentctid='" + ctid + "',marked_for_delete=true from (select \"ctid\" as \"MyCtid\" ");
                                        sb.Append($" from \"{schemaName}\".\"{tableName}\" where ");
                                        int loop_count = 0;
                                        ...
                                        sb.Append($" and ctid<>'" + ctid + "')b WHERE a.\"myctid\"=b.\"MyCtid\" ");
                                    ...
                                    if (dbResult != null && dbResult.Count() > 0)
                                    {
                                        tableDataSize = dbResult.FirstOrDefault();
                                        UpdateSyncInfo(id: (int)connectorConfig.connectorId, ccid: connectorConfig.ccid, dedup_process_count: (int)tableDataSize.total_rows);
                                        WriteProcessLogToConsole(connectorConfig.connectorName, $"Dedup process count => {(int)tableDataSize.total_rows}");
                                    }
        ...
        public async Task DeDupRowsFromDatabaseTable(IJobCancellationToken jobCancelToken, int connectorId, string ccid)
        {
            try
            {
                ...
                for (int src = 0; src < connectorConfig.multipleDBConfigs.Count; src++)
                {
                    ...
                    if (connectorConfig.dedupSourceType == SourceType.Copy_Source_data_to_Destination_and_Remove_Duplicates_from_Destination || connectorConfig.dedupSourceType == SourceType.Merge_Table_A_Data_to_Table_B_and_Remove_Duplicates_from_Table_B)
                    {
                        if (connectorConfig.dbConfig_compare.table_type != SelectedTableType.Create_New_Table && connectorConfig.dbConfig_compare.new_table_name == null)
                        {
                        ...
                        {
                            compareTblCols = GetPGDatabaseTableColumns(connectorConfig.dbConfig_compare, connectorConfig.dbConfig_compare.new_table_name, connectorConfig.dbConfig_compare.db_schema);
                        }
                                            ...
                                            if (connectorConfig.dedupSourceType == SourceType.Remove_Duplicates_from_a_Single_Table && connectorConfig.dedup_type == DedupType.Simulate_and_Verify)
                                            {
                                                try
                                                {
                                                    ...
                                                    if (pgRows != null && pgRows.Count() > 0)
                                                    {
                                                        ...
                                                        {
                                                            var properties = (from key in pgRows.ElementAt(0).Keys join b in fetchDestColumns on key equals b select key).ToList();
